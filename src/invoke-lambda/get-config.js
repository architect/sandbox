...
module.exports = function readConfigFor (cwd, callback) {
  ...
  fs.readFile(arcConfig, function done (err, data) {
    if (err) {
      ...
      let awsRuntime = arc.aws.find(tuple => Array.isArray(tuple) && tuple.includes('runtime'))
...
function getTimeout (arc) {
  ...
  let ttl = arc && arc.aws.find(tuple => Array.isArray(tuple) && tuple.includes('timeout'))
  // Valid ttl is between 1 and 900 seconds (15 minutes)
  if (ttl && ttl[1] > 0 && ttl[1] <= 900) {
