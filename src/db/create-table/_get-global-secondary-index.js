module.exports = function _getGSI(name, indexes) {
  return process.env.DEPRECATED? deprec(name, indexes) : gsi(name, indexes)
}

function gsi(name, indexes) {
  ...
  if (actual.length >= 1) {
    return actual.map(idx=> {
      ...
      let IndexName = hasOne? `${keys[0]}-index` : `${keys[0]}-${keys[1]}-index`
